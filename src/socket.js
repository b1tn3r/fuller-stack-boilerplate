...
const socket = socketIOClient(window.location.host);        // makes the connection to the server at host 'localhost:3000' and uses the socket.io-client framework to be able to handle client specific functionality and the socket.io connection will automatically disconnect when the client refreshes the page or navigates from it


module.exports = (store) => {           // store is passed into our socket config function so we can use it to send out store data and then dispatch actions to the store as a response to incoming data from the server
    ...
    socket.on(IO_CONNECT, () => {                   // creates the connection to the server upon loading the page since this function just starts after the App is hydrated in client.js, so this makes the 'connect' event
        ...
        socket.emit(IO_CLIENT_HELLO, `Hello Button Msg - 
                    ${store.getState().main.hello}`);             // sends a message to the server of event type IO_CLIENT_HELLO, which the server will receive it by referring to the event name, and we send back our 'hello' String that is currently stored in 'main.hello' of our store which we retrieve with 'store.getState()' to get the current data of the state, and this will allow us to use whatever is currently displayed on our page since both this store and the important Strings and objects on our page get their data from the same central current state in our store
    });
