...
const browserConfig = {
    ...
    plugins: dev ? [    // DEV
        ...
        /*new webpack.DefinePlugin({                             // do not need anymore since we have 'better-npm-run' to set global variables in for our 'process.env' and then we have config to set the variables in our project so there is no need to set global variables with DefinePlugin anymore... This DefinePlugin placed in the client config, will be able to have its global variables set here used and referenced anywhere in our client bundled code
            ...
            'process.env.BROWSER': JSON.stringify(true),                // to be able to use 'window' and 'window.$' jquery in the App, we will need to configure it so the server side rendering does not try to use these places where we call 'window' in the App or '$' (a global variable on window.$) or 'navigator' on window.navigator, and this also includes 'document' as well which is a separate global variable from 'window', in which both the DOM and window within the DOM 'document' need to either both be configured for our SSR with a library like 'server-components' or other methods talked about here: "https://medium.com/@pimterry/building-a-server-rendered-map-component-part-2-using-client-side-libraries-6f1bb751f31c" for complex libraries like Leafly which requires a 'window', 'document' and 'navigator' global variables for SSR to render a String from the App and pass back a static view of the Leafly map, or we could take a shortcut around this for our App here which uses less complex libraries and we are mainly just utilizing jquery $('body') in our App's rendered code/JSX. Here we use DefinePlugin to define a "BROWSER" global variable that we can use to tell us whether the App is being rendered in SSR or by the client, where "BROWSER" will be 'undefined' if SSR is rendering the App at the time so we can use "BROWSER" global var in a conditional to put a placeholder for 'window', 'document' or '$' such as 'global.window = {};' which will allow us to temporarily define window, document, or whatever else to a blank object so it will not throw errors like "window is not defined" and such. Instead it will use the blank {} and render nothing for the SSR. This method is choppy, but is the only thing needed for now until more complex libraries are used on the SSR and client.
        }),*/
    ]
};
